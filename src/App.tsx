...
import { MessageCircle, Send, ChevronDown, ChevronUp, Heart, Share } from "lucide-react";
...
const App: React.FC = () => {
  const { posts, fetchPosts, addComment, toggleCommentsVisibility, toggleDescriptionVisibility } = useStore();
            ...
            >
              <Card className="group relative overflow-hidden backdrop-blur-sm bg-[#1E293B] text-white border border-[#2C3E50] shadow-lg hover:shadow-2xl transition-all duration-300">
                ...
                />
                 ...
                 <p className="text-[#AAB4C2] mb-2">{post.reactions.likes} Likes {post.views} Views</p>
                  <h2 className="text-xl font-semibold mb-3">{post.title}</h2>
                  <p className=" text-[#AAB4C2] mb-2">
                    {post.showFullDescription ? post.body : `${post.body.substring(0, 100)}`} 
                    
                    {post.tags.map((t) => <span className="text-blue-400  cursor-pointer">{" "} #{t}</span>)}{!post.showFullDescription && "..."} {" "}

                    <span className=" underline cursor-pointer" onClick={() => toggleDescriptionVisibility(post.id)}>
                  ...
                  <div className="flex  mb-3">
                      <Heart className="size-12 p-3 cursor-pointer hover:bg-[#1B2430] rounded-lg" />
                      <MessageCircle className="size-12 p-3 cursor-pointer  hover:bg-[#1B2430] rounded-lg" />
